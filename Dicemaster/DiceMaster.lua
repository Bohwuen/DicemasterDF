...
local GUILDS_ALLOWED = {
	["Aventurero"] = true;
	["Lobos de Azeroth"] = true;
	["Leyendas del Olvidado"] = true;
	["Mariposa Nocturna"] = true;
	["El Sueño Lúcido"] = true;
}
...
StaticPopupDialogs["DICEMASTER4_SETHEALTHMAX"] = {
  text = "Fijar valor máximo de salud:",
  button1 = "Aceptar",
  button2 = "Cancelar",
  OnShow = function (self, data)
...
StaticPopupDialogs["DICEMASTER4_SETMANAMAX"] = {
  text = "Fijar valor de mana máximo:",
  button1 = "Aceptar",
  button2 = "Cancelar",
  OnShow = function (self, data)
	local manaType = Profile.manaType or "Mana";
...
function Me.IsOfficer()
	...
	if Me.PermittedUse() and guildRankIndex < 2 then
		return true
	end
	return false
end
...
--
local TRAIT_USAGE = {
	["USE1"] = "1 Uso"; ["USE2"] = "2 Usos"; ["USE3"] = "3 Usos";
	["PASSIVE"] = "Pasivo";
...
function Me.FormatUsage( usage, name )
	...
	local text = TRAIT_USAGE[usage] or "<Uso desconocido>"
	...
	local manaType = Profile.manaType or "Mana";
	if manaType == "RunicPower" then manaType = "Poder Rúnico" end
	-- sub mana
...
local TRAIT_CAST_TIME = {
	["INSTANT"] = "Instantaneo"; ["CHANNELED"] = "Canalizado"; ["TURN1"] = "1 turno de lanzamiento"; ["TURN2"] = "2 turnos de lanzamiento"; ["TURN3"] = "3 turnos de lanzamiento"; ["TURN4"] = "4 turnos de lanzamiento"; ["TURN5"] = "5 turnos de lanzamiento";
}

function Me.FormatCastTime( castTime )
	local text = TRAIT_CAST_TIME[castTime] or "Instantaneo"
	return text
end
...
--
local TRAIT_RANGE = {
	["NONE"] = "(Ninguno)"; ["MELEE"] = "Rango a Melee";  ["10YD"] = "10 yd range"; ["20YD"] = "20 yd range"; ["30YD"] = "30 yd range"; ["40YD"] = "40 yd range"; ["50YD"] = "50 yd range"; ["60YD"] = "60 yd range"; ["70YD"] = "70 yd range"; ["80YD"] = "80 yd range"; ["90YD"] = "90 yd range"; ["100YD"] = "100 yd range"; ["UNLIMITED"] = "Unlimited range"; 
}
...
local TRAIT_COOLDOWN = {
	["NONE"] = "(Ninguno)"; ["15S"] = "15 seg"; ["20S"] = "20 seg"; ["30S"] = "30 seg"; ["1M"] = "1 min"; ["2M"] = "2 min"; ["3M"] = "3 min"; ["4M"] = "4 min"; ["5M"] = "5 min"; ["10M"] = "10 min"; ["15M"] = "15 min"; ["20M"] = "20 min"; ["30M"] = "30 min"; ["1H"] = "1 hora"; ["2H"] = "2 horas"; ["3H"] = "3 horas"; ["4H"] = "4 horas"; ["5H"] = "5 horas"; ["1D"] = "1 dia"; ["2D"] = "2 dias"; ["3D"] = "3 dias"; ["4D"] = "4 dias"; ["5D"] = "5 dias"; ["1W"] = "1 semana"; ["1T"] = "1 turno"; ["2T"] = "2 turnos"; ["3T"] = "3 turnos"; ["4T"] = "4 turnos"; ["5T"] = "5 turnos"; ["6T"] = "6 turnos";
}

function Me.FormatCooldown( cooldown )	
	local text = TRAIT_COOLDOWN[cooldown] or ""
	if ( cooldown ~= "NONE" and text ~= "" ) then
		text = text .. " enfriamiento"
	end
	return text
end
...
-- Función para ajustar el mana dentro de los límites establecidos.
function Me.ModMana(delta)
    local store = Profile

    -- Verificar si el delta es un número válido
    if type(delta) ~= "number" then
        print("Error: El delta debe ser un número.")
        return
    end

    -- Calcular el nuevo valor de mana
    local nuevoMana = store.mana + delta

    -- Asegurarse de que el nuevo valor esté dentro de los límites
    nuevoMana = Me.Clamp(nuevoMana, 0, store.manaMax)

    -- Aplicar el cambio
    store.mana = nuevoMana
...
function Me.RefreshChargesFrame( tooltip, color )
	...
	if tooltip then
		...
		Me.SetupTooltip( DiceMasterChargesFrame.bar, nil, chargesPlural, 
			nil, nil, nil, nil,
			Profile.charges.tooltip.."|n|cFF707070<Click izquierdo para añadir "..chargesPlural..">|n"
			.."<Click derecho para quitar "..chargesPlural..">")
	end
	...
	if not Profile.charges.symbol:find("charge") then
		...
		DiceMasterChargesFrame.bar2:ApplyTextures( Profile.charges.symbol, Profile.charges.name, Profile.charges.tooltip .. "|n|cFF707070<Click izquierdo para añadir " .. chargesPlural .. ">|n" .. "<Click derecho para quitar " .. chargesPlural .. ">", Profile.charges.count, Profile.charges.color, Profile.charges.flash )
	...
	elseif button == "RightButton" then
		...
		if ( hasCost ) then
			...
			if cost > Profile.mana then
				...
				PlaySound(1428)
				UIErrorsFrame:AddMessage( "No hay suficiente mana", 1.0, 0.0, 0.0 ); 
				return
			else
		...
		if cooldown ~= "NONE" then
			if traitButton.cooldown:GetCooldownDuration() == 0 and not traitButton.cooldown.text:IsShown() and not traitButton.notCastable then
				...
				ExecuteEffects( Profile.traits[traitButton.traitIndex]["efectos"], traitButton.traitIndex )
		...
		elseif not traitButton.notCastable then
			...
			updateTraitUses( traitButton )
			PlaySound(1428)
			UIErrorsFrame:AddMessage( "El rasgo no está listo aun.", 1.0, 0.0, 0.0 ); 
		end
	end
end
...
function Me.DiceButton_ExpandMenu( self )
	...
	if not( self.FadeIn ) then
		self.FadeIn = C_Timer.NewTicker( 0.05, function()
			...
			local y = radius * math.sin(((index-1)/numSkills) * (2*math.pi)) -250 ; 
...
function Me:Initialize()
    -- Aquí puedes poner otras inicializaciones del addon
    -- Por ejemplo, cargar configuraciones, inicializar variables, etc.

    -- Llama a la función para actualizar el EditBox según la configuración actual
    Me.UpdateDiceEditBox()
end

-- Función para actualizar el EditBox
function Me.UpdateDiceEditBox()
    -- Acceder al EditBox con el nombre completo
    local editBox = _G["DiceMasterPanelDice"]
    if not editBox then
        print("EditBox 'DiceMasterPanelDice' no encontrado")
        return
    end
...
local function OnAddonLoaded(event, addonName)
    if addonName == "DiceMaster" then
        -- Inicializa el addon
        Me:Initialize()
    end
end

-- Crear un frame para registrar el evento ADDON_LOADED
