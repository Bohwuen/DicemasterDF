...
local function GetSkillByGUID( guid )
	for i = 1, #Me.inspectData[Me.statInspectName].skills do
		if Me.inspectData[Me.statInspectName].skills[i].guid == guid then
			return Me.inspectData[Me.statInspectName].skills[i];
		end
	end
	return
end
...
function Me.StatInspector_Update()
	...
	if store and store.alignment then
		UIDropDownMenu_SetText( DiceMasterStatInspectorSkillFrameAlignmentDropdown, "|cFFFFD100Alineamiento:|r " .. store.alignment )
	end
