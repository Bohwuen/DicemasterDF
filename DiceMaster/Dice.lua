...
local function FormatDiceMasterRoll_v2( name, you, count, sides, mod, rolls, rollType, logRoll )
	...
	--	end
		...
		for i = 2,count do
			if i == count then
				if count > 2 then
					rollstring = rollstring .. ", y " .. ColoredRoll( rolls[i], sides )
				else
					rollstring = rollstring .. " y " .. ColoredRoll( rolls[i], sides )
				end
			else
	...
	if not you then
		if rollType then
			...
			return "(" .. rollType .. ") " .. name .. " tira " .. rollstring
		end
		return name .. " tira " .. rollstring
	else
		if rollType then
			Me.SecretEditor_OnEvent( "ROLL", rollType )
			Me.SecretEditor_OnEvent( "ROLL"..rollquality, rollType )
			...
			return "(" .. rollType .. ") Tiras " .. rollstring
		end
		return "Tiras " .. rollstring
	end
end
...
function Me.Roll( dice, rollType )
	...
	if type( dice ) ~= "string" then 
		return UIError( "Formato de dado inválido." )
	end
	
	local count, sides, modtype, mod = dice:match("^%s*(%d*)[dD](%d+)([+-]?)(%d*)%s*$")
	
	if not count then return UIError( "Formato de dado inválido." ) end
	...
	if count == 0 then    return UIError( "Debes tener al menos un dado." )              end 
	if count > 10 then    return UIError( "Solo puedes tirar 20 dados a la vez." )         end 
	if sides < 2 then     return UIError( "Debe tener al menos dos lados." )                end 
	if sides > 13476 then return UIError( "Los dados no pueden tener más de 13476 lados." ) end
...
local function RollPartyFilter( self, event, msg, ... )
	...
	if msg:match( "^<DiceMaster> %S+ Tira" ) then
...
function Me.Dice_OnRollMessage( data, dist, sender )
	...
	if sender == UnitName("player") then return end
		...
		if sender == (myName.."-"..myRealm) then return end
