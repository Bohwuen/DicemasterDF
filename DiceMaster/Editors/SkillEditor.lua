...
StaticPopupDialogs["DICEMASTER4_CREATESKILLHEADER"] = {
  text = "Nombre de Categoría de habilidad:",
  button1 = "Aceptar",
  button2 = "Cancelar",
  ...
  OnAccept = function (self)
	...
	if GetHeaderPositionByName( data ) then
		UIErrorsFrame:AddMessage( "El nombre de esta Categoría de habilidad ya existe.", 1.0, 0.0, 0.0 );
		return
	end
	...
	UIDropDownMenu_SetText( DiceMasterSkillEditor.SkillType, "|cFFFFD100Categoría de Habilidad:|r " .. data )
  end,
  hasEditBox = true,
  timeout = 0,
  whileDead = true,
...
function Me.SkillEditorType_OnClick(self, arg1, arg2, checked)
	...
	UIDropDownMenu_SetText( DiceMasterSkillEditor.SkillType, "|cFFFFD100Categoría de Habilidad:|r " .. arg1 )
end
...
function Me.SkillEditorModifiers_OnClick(self, arg1, arg2, checked)
	...
	UIDropDownMenu_SetText( DiceMasterSkillEditor.SkillModifiers, "|cFFFFD100Modificador de habilidad:|r " .. modifiersList )
end
	...
	for i = 1, #Me.Profile.skills do
		if Me.Profile.skills[i].type == "header" then
			...
			if not ( lastCategory ) then
				skillList[ "Miscelanea" ] = {}
				lastCategory = "Miscelanea";
			end
...
function Me.SkillEditor_CreateSkill()
	...
	local skillType = UIDropDownMenu_GetSelectedValue( DiceMasterSkillEditor.SkillType ) or "Miscelanea";
...
function Me.SkillEditor_ClearAllFields()
	...
	UIDropDownMenu_SetText( editor.SkillType, "|cFFFFD100Categoría de Habilidad:|r Miscelanea" )
	UIDropDownMenu_SetText( editor.SkillModifiers, "|cFFFFD100Modificadores de habilidad:|r (None)" )
	...
	for i = 1, #Me.Profile.skills do
		if Me.Profile.skills[i].type == "header" then
			...
			UIDropDownMenu_SetText( DiceMasterSkillEditor.SkillType, "|cFFFFD100Categoría de Habilidad:|r " .. Me.Profile.skills[i].name )
			break;
		end
	end
