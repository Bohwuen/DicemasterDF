...
StaticPopupDialogs["DICEMASTER4_EXPORT"] = {
  text = "Copia el código a continuación:",
  button1 = "Ok",
  OnShow = function (self, data)
...
StaticPopupDialogs["DICEMASTER4_IMPORTTRAIT"] = {
  text = "Pega el código de importación y pulsa 'Importar' para importar el rasgo.|n|nEsto sobreescribirá tu rasgo actual!",
  button1 = "Importar",
  button2 = "Cancelar",
  OnShow = function (self, data)
  ...
  OnAccept = function (self, data)
	...
	if not data or data == nil or data == "" then
		UIErrorsFrame:AddMessage( "Código de importación inválido.", 1.0, 0.0, 0.0 );
		return
	end
	...
	if T and type( T ) == "table" and T.name and T.usage and T.desc and T.icon then
		...
		Me.PrintMessage( "|T" .. T.icon .. ":16|t " .. T.name .. " ha sido importado.", "SYSTEM" )
	else
		UIErrorsFrame:AddMessage( "dato corrupto localizado imposible importar.", 1.0, 0.0, 0.0 );
	end
	TraitUpdated()
...
StaticPopupDialogs["DICEMASTER4_IMPORTITEM"] = {
  text = "Pega el código de importación y pulsa 'Importar' para importar el objeto.",
  button1 = "Importar",
  button2 = "Cancelar",
  OnShow = function (self, data)
    self.editBox:SetText( "" )
  ...
  OnAccept = function (self, data)
	...
	if T and type( T ) == "table" and T.name and T.stackCount and T.stackSize and T.guid then
		...
		Me.PrintMessage( itemLink .. " Ha sido importado.", "SYSTEM" )
...
StaticPopupDialogs["DICEMASTER4_DELETETRAIT"] = {
  text = "Estas seguro de querer borrar este rasgo??",
  button1 = "Si",
  button2 = "No",
  OnAccept = function (self, data)
	Me.TraitEditor_SelectIcon( "Interface/Icons/inv_misc_questionmark" )
    Me.editor.scrollFrame.Container.traitName:SetText("Nombre de rasgo")
	Me.editor.scrollFrame.Container.descEditor:SetText("Escribe la descripción de tu rasgo aqui.")
	Me.TraitEditor_SaveName()
	Me.TraitEditor_SaveDescription()
	if not Me.editing_trait then return end
	
	Me.PrintMessage( "|T" .. Profile.traits[Me.editing_trait].icon .. ":16|t " .. Profile.traits[Me.editing_trait].name .. " ha sido borrado.", "SYSTEM" )
...
function Me.PetEditor_Refresh()
	...
	if Profile.pet.enable and Profile.pet.isComplex then
		...
		if Profile.pet.item then
			DiceMasterPetFramePetItemSlot:SetPetItem();
		else
			DiceMasterPetFramePetItemSlot:SetTexture(nil);
		end
		...
		DiceMasterPetFrame.petIcon:Disable();
	else
		...
		DiceMasterPetFrame.petIcon:Enable();
		--DiceMasterPetFrame.PetItemSlot:SetTexture(nil);
	end
end
...
function Me.PetEditor_WashPet()
	...
	if not DiceMasterPetFrame.petModelScene.bubbles then
		DiceMasterPetFrame.petModelScene.bubbles = DiceMasterPetFrame.petModelScene:CreateActor("bubbles", "DiceMasterPetFrameActorTemplate");
		DiceMasterPetFrame.petModelScene.bubbles:SetModelByFileID(165411);
		DiceMasterPetFrame.petModelScene.bubbles:SetScale(0.3);
	end
	DiceMasterPetFrame.petModelScene.bubbles:Show();
	DiceMasterPetFrame.petModelScene.bubbles:SetPosition(0,0,0);
...
function Me.PetEditor_CleanPetDroppings()
	if not DiceMasterPetFrame.petModelScene.broom then
		...
		DiceMasterPetFrame.petModelScene.broom:SetModelByFileID(4382564);
		DiceMasterPetFrame.petModelScene.broom:SetScale(0.3);
	...
	DiceMasterPetFrame.petModelScene.broom:SetPosition(-1,-3,0);
	...
	DiceMasterPetFrame.petModelScene:SetScript("OnUpdate", function( self, elapsed )
		...
		DiceMasterPetFrame.petModelScene.broom:SetPosition(-1, y+0.008, 0);
	end)
	...
	if Profile.pet.numBMs then
		...
		local ticker = C_Timer.NewTicker( 1, function()
			...
			if DiceMasterPetFrame.petModelScene["poodad"..index] then
				UIFrameFadeOut( DiceMasterPetFrame.petModelScene["poodad"..index], 1, 1, 0 );
			end
...
-------------------------------------------------------------------------------
-- Handler for adding pet "furniture."
--
function Me.PetEditor_AddFurniture( fileID )
	if not( DiceMasterPetFrame.petModelScene.objects ) then
		DiceMasterPetFrame.petModelScene.objects = {};
	end
	if #DiceMasterPetFrame.petModelScene.objects < 4 then
		local object = DiceMasterPetFrame.petModelScene:CreateActor("broom", "DiceMasterPetFrameActorTemplate");
		object:SetModelByFileID(fileID);
		object:SetScale(0.2);
		object:SetAlpha(1);
		object:SetPosition(-1,-3 + (#DiceMasterPetFrame.petModelScene.objects+1),0);
		object:Show();
		tinsert( DiceMasterPetFrame.petModelScene.objects, object );
	end
end
...
function Me.SkillFrame_IncreaseButton_OnClick( self, button )
	...
	if skillAuthor == UnitName("player") or skillCanEdit then
		...
		UIErrorsFrame:AddMessage( "No puedes hacer eso.", 1.0, 0.0, 0.0 );
	end
	Me.SkillFrame_UpdateSkills()
end
...
function Me.SkillFrame_CreateWoWJdR()
	Profile.skills = {};
	local skill = {
		name = "Atributos";
		type = "header";
	...
	for k, v in pairs( Me.AttributeListWoWJdR ) do
		local skill = {
			name = k;
			icon = v.icon;
			desc = v.desc;
			...
			author = UnitName("player");
			expanded = true,
			showOnMenu = v.showOnMenu;
			guid = k;
			canEdit = true;
			skillPosition = v.skillPosition;
		}
		tinsert(Profile.skills, skill)
	end
	
	for k, v in pairs( Me.RollListWoWJdR ) do
		local skill = {
			name = k;
			type = "header";
			author = UnitName("player");
...
function Me.SkillFrame_ExportSheet()
	...
	DiceMasterExportDialog.titleText = "Exportar habilidades";
...
function Me.SkillFrame_ImportSheet()
	...
	if not( T and Me.ValidateSkillSheet(T)) then 
		...
		UIErrorsFrame:AddMessage( "Código de importación inválido. 2", 1.0, 0.0, 0.0 );
		return 
	end
...
function Me.SkillFrameTemplate_OnLoad(frame, level, menuList)
	local info = UIDropDownMenu_CreateInfo();
	info.text = "Plantilla de habilidades";
	info.isTitle = true;
	info.notClickable = true;
	info.notCheckable = true;
	UIDropDownMenu_AddButton(info);
	info.text = "Plantilla por defecto";
	info.tooltipTitle = "Default";
	info.tooltipText = "La planyilla estandar de atributos, habilidades y tiradas de salvación.";
	info.tooltipOnButton = true;
	info.isTitle = false;
	...
	info.disabled = false;
	UIDropDownMenu_AddButton(info);
	info.text = "Plantilla WoW JdR";
	info.tooltipTitle = "WoW JdR";
	info.tooltipText = "La planyilla de WoW JdR de atributos, habilidades y tiradas de salvación.";
	info.tooltipOnButton = true;
	info.isTitle = false;
	info.notClickable = false;
	info.func = Me.SkillFrame_CreateWoWJdR;
	info.disabled = false;
	UIDropDownMenu_AddButton(info);
	info.text = CreateAtlasMarkup("editmode-new-layout-plus") .. " |cFF00FF00Importar|r";
	info.tooltipTitle = CreateAtlasMarkup("editmode-new-layout-plus") .. " |cFF00FF00Importar|r";
	info.tooltipText = "Usa un código apara importar una planilla hecha por otro jugador.";
	info.arg1 = 0;
	info.value = 0;
	info.notCheckable = true;
	info.isTitle = false;
...
function Me.SkillFrameAlignment_OnLoad(frame, level, menuList)
	...
	elseif menuList then
		for i = 1, #Me.AlignmentList[menuList] do
			...
			info.tooltipText = Me.AlignmentList[menuList][i].desc .. "|n|nEjemplos: |cFFFFFFFF" .. Me.AlignmentList[menuList][i].examples[1];
