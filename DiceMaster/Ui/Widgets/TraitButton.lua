...
function Me.OpenTraitTooltip( owner, trait, index )
	...
	if owner and owner:GetParent():GetName() == "DiceMasterInspectFrame" and not UnitIsUnit("target", "player") and Me.IsOfficer() then
		...
		if not found then
			usable = "<Click derecho para aprobar>|n"
		else
			usable = "<Click derecgo para retirar aprobación>|n"
		end
	end
	
	if trait.effects and next(trait.effects) and owner and owner:GetParent():GetName() == "DiceMasterChargesFrame" then
		usable = usable .. "<Click derecho para usar>|n"
	end
	
	if owner and owner:GetParent():GetName() == "DiceMasterChargesFrame" then
		GameTooltip:AddLine( "<Click izquierdo para editar>", 0.44, 0.44, 0.44, true )
	end
	
	GameTooltip:AddLine( usable .. "<Shift+Click para Link al Chat>", 0.44, 0.44, 0.44, true )
	
	if trait.officers and Me.PermittedUse() then
		local approval
		if trait.officers[2] then
			approval = "|TInterface/AddOns/DiceMaster/Texture/trait-approved:14:14:0:0:32:32:2:14:2:14|t Aprobado por " .. trait.officers[1] .. " y " .. trait.officers[2]
			GameTooltip:AddLine( approval, 0, 1, 0, true )
		elseif trait.officers[1] then
			approval = "|TInterface/AddOns/DiceMaster/Texture/trait-approved:14:14:0:0:32:32:2:14:18:30|t Aprobado " .. trait.officers[1]
